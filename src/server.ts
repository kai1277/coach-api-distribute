...
async function retrieveTopK(
  ...
  try {
    ...
    if (error) {
      console.warn(
        "[retrieveTopK] rpc error (fallback to empty):",
        error.message
      );
      return [];
    }
...
async function upsertKnowledgeDoc(params: {
  ...
  if (docIns.error || !docIns.data) {
    throw new Error(
      `[upsertKnowledgeDoc] insert doc failed: ${docIns.error?.message}`
    );
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (ins.error) {
      console.warn(
        "[upsertKnowledgeDoc] insert chunk error:",
        ins.error.message
      );
    }
  }
...
async function ragCasebook(
  ...
  for (const strength of strengths_top5) {
    ...
    if (error) {
      console.warn(
        `[ragCasebook] error for strength "${strength}":`,
        error.message
      );
      continue;
    }
  ...
  if (strengths_top5.length === 0) {
    ...
    if (error) {
      console.warn("[ragCasebook] ", error.message);
      return [];
    }
    return data ?? [];
  }
...
async function ragQuestionTemplates(
  context: {
    hypotheses?: string[];
  ...
  if (error) {
    console.warn("[ragQuestionTemplates] ", error.message);
    return [];
  }
  return data ?? [];
}
...
function updatePosterior(prev: Record<string, number> | null, answer: string) {
  ...
  (Object.keys(base) as Array<keyof typeof base>).forEach(
    (k) => (norm[k] = base[k] / sum)
  );
  return norm;
}
  ...
  const answersContext =
    answers.length > 0
      ? answers
          .map(
            (a, i) =>
              `${i + 1}. Q: ${a.question_id ?? "(unknown)"} → A: ${
                a.answer_text || a.answer || "(未回答)"
              }`
          )
          .join("\n")
      : "(まだ回答なし)";
...
{ "questions": [ { "theme": "<統合的テーマ名(空でも可)>", "text": "<日本語の質問文>" } ] }`;
  ...
  const out: Question[] = arr
    ...
    .map(
      (
        q: { id?: string; theme?: string; text?: string },
        i: number
      ): Question => ({
...
async function runInterviewer(opts: {
  ...
  const qtemps = await ragQuestionTemplates(
    { hypotheses, last_answers: qa_pairs },
    4
  );
  const qtempContext = qtemps
    .map(
      (t: any, i: number) =>
        `T${i + 1}: ${t.template} | goal:${t.goal} | followups:${JSON.stringify(
          t.followups
        )}`
    )
    .join("\n");
...
出力: {"question":{"text":"...", "goal":"...", "template_id":"Q-... (あれば)"}}`;
  ...
  return {
    question: {
      ...
      text:
        String(q?.text ?? "").trim() || "最近の仕事で一番うまくいったことは？",
      goal: String(q?.goal ?? ""),
    },
  };
}
...
出力:
{
  ...
  return {
    ...
    management: {
      ...
      dont: Array.isArray(o?.management?.dont)
        ? o.management.dont.slice(0, 3)
        : [],
    },
    next_week_plan: Array.isArray(o?.next_week_plan)
      ? o.next_week_plan.slice(0, 3)
      : [],
  };
}
...
function buildBasicInfo(row: UserRowWithProfile): {
  age?: string;
  gender?: string;
  hometown?: string;
} {
...
app.post("/api/users", async (req, res) => {
  const { email, display_name, username, department, role, goal } =
    req.body ?? {};
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert(insertData)
      .select(
        "id, email, display_name, department, role, goal, strength_1, strength_2, strength_3, strength_4, strength_5, age, gender, hometown, created_at, updated_at"
      )
      .single();
...
app.post("/api/users/login", async (req, res) => {
  ...
  try {
    ...
    if (email) {
      ...
      console.log(
        "[LOGIN] Searching for user with username:",
        normalizedUsername
      );
...
app.patch("/api/users/:id", async (req, res) => {
  ...
  try {
    ...
    if (basicInfo !== undefined) {
      const info =
        typeof basicInfo === "object" && basicInfo !== null
...
app.post("/api/sessions", async (req, res) => {
  try {
    const { transcript, context, strengths_top5, demographics, userId } =
      req.body ?? {};
    if (!transcript)
    ...
    if (typeof userId === "string" && userId.trim().length > 0) {
      ...
      return res
        .status(400)
        .json({ error: "userId must be a valid string or null" });
    }
    ...
    if (error) {
      if ((error as any)?.code === "23503") {
        return res
          .status(400)
          .json({ error: "user not found for provided userId" });
      }
    ...
    const next_steps: string[] = Array.isArray(meta.next_steps)
      ? meta.next_steps
      : [];
...
app.get("/api/sessions", async (req, res) => {
  try {
    ...
    const sessions = (data ?? []).map((session) => ({
      ...
      title: session.title,
      summary:
        typeof session.metadata?.summary === "string"
...
app.post("/api/sessions/:id/seed-questions", async (req, res) => {
  try {
    ...
    st.recentTexts = Array.from(
      new Set([...newTexts, ...st.recentTexts])
    ).slice(0, 10);
...
app.get("/api/sessions/:id/questions/next", async (req, res) => {
  try {
    ...
    if (st.asked === 0) {
      const seeds: any[] = Array.isArray(meta.seed_questions)
        ? meta.seed_questions
        : [];
      const first = seeds[0];
      if (first && typeof first.text === "string" && first.text.trim()) {
        q = {
          id: first.id ?? `QSEED_${Date.now()}`,
    ...
    // recentTexts に積む
    if (q?.text) {
      st.recentTexts = Array.from(new Set([q.text, ...st.recentTexts])).slice(
        0,
        10
      );
    }
...
app.post("/api/sessions/:id/answers", async (req, res) => {
  try {
    ...
    const row = await supabase
      .from("sessions")
      .select("asked_count, max_questions, metadata")
      .eq("id", id)
      .single();
    ...
    console.log(
      `[POST /answers] session=${id}, asked=${asked_count}, max=${max_questions}`
    );

    // 3) 収束判定
    if (asked_count >= max_questions) {
      console.log(
        `[POST /answers] Reached max questions, generating conclusion...`
      );
      const rawPairs = await fetchQAPairs(id);
    ...
    console.log(
      `[POST /answers] Generated question: id=${inter.question.id}, text="${inter.question.text}"`
    );
    ...
    st.loop = {
      ...
      maxQuestions:
        typeof maxQuestions === "number"
          ? Math.min(Math.max(maxQuestions, 3), 12)
          : st.loop.maxQuestions,
      minQuestions:
        typeof minQuestions === "number"
          ? Math.min(Math.max(minQuestions, 0), 10)
          : st.loop.minQuestions,
    };

    const prev =
      (await supabase.from("sessions").select("metadata").eq("id", id).single())
        .data?.metadata ?? {};
    await supabase
      .from("sessions")
...
app.post("/api/sessions/:id/actions", async (req, res) => {
  ...
  try {
    ...
    function clamp(s: string, max = 2000) {
...
- JSON 以外の文字は出力しない`;
    ...
    const asstTurn = await supabase
      ...
      .single();
    if (asstTurn.error)
      return sendErr(res, 500, "failed to insert assistant turn");
...
app.post("/api/knowledge/import", async (req, res) => {
  try {
    ...
    if (!Array.isArray(chunks) || chunks.length === 0) {
      return res
        .status(400)
        .json({ error: "chunks is required (non-empty array)" });
    }
    ...
    return res
      .status(201)
      .json({ ok: true, doc_id: result.doc_id, chunks: chunks.length });
  } catch (e: any) {
